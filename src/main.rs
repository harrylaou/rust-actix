...
async fn main() -> std::io::Result<()> {
    ...
    let time_series: TimeSeries = get_time_series().await.map_err(|err| {
        eprintln!("Problem TimeSeries: {}", err);
        ErrorKind::Other
    })?; //todo proper error type

    let time_series_result_state: Data<RwLock<TimeSeries>> =
        web::Data::new(RwLock::new(time_series));
    ...
    let mut server = HttpServer::new(move || {
        App::new()
            .app_data(time_series_result_state.clone())
